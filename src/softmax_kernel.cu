...
void launch_attn_softmax(float *inp, const float *attn_mask,
  ...
  cudaMalloc((void **)&dk, attn_mask_size);

  cudaMemcpy(d_soft_inp, inp, inp_size, cudaMemcpyHostToDevice);
  cudaMemcpy(dk, attn_mask, attn_mask_size, cudaMemcpyHostToDevice);

  dim3 grid_dim(1, batch_size, nhead);
  if (to_len <= 32) {
    ker_attn_softmax_lt32<float, 32, 1><<<grid_dim, 32, 0, stream>>>(
        d_soft_inp, dk, from_len, to_len, mask_future);
  } else if (to_len <= 64) {
  ...
  cudaMemcpy(inp, d_soft_inp, inp_size, cudaMemcpyDeviceToHost);
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  // BEGIN ASSIGN4_1_2
  int float_size = sizeof(float);
  int soft_inp_size = softmax_len * rows * float_size;
  int out_grad_size = softmax_len * rows * float_size;
