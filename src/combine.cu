...
__global__ void reduceKernel(
   ...
   */
    ...
    if (ordinal < out_size) {
      ...
      // for (int i = 0; i < shape_size; i++) {
      //   printf("a_strides[%d]: %d\n", i, a_strides[i]);
      // }

      //printf("a_strides[reduce_dim]: %d\n", a_strides[reduce_dim]);
      for (int reduced_dim_idx = 0; reduced_dim_idx < a_shape[reduce_dim]; reduced_dim_idx++){
        a_index[reduce_dim] = reduced_dim_idx;
...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    int num_dims = 3;
    int out_pos = index_to_position(out_index, out_strides, num_dims);
    ...
    float out_value = 0.0;
    int a_strides_all[3] = {a_batch_stride, a_strides[1], a_strides[2]};
    int b_strides_all[3] = {b_batch_stride, b_strides[1], b_strides[2]};
    
    for (int phase = 0; phase < (n+TILE-1)/TILE; phase++) {
      // Load matrix A into shared memory
      if ((phase * TILE + threadIdx.x < n) && (row < m)) {
        int a_row = row;
        int a_col = phase * TILE + threadIdx.x;
        int a_index[3] = {batch, a_row, a_col};
        int a_pos = index_to_position(a_index, a_strides_all, num_dims);
      ...
      if ((phase * TILE + threadIdx.y < n) && (col < p)) {
        int b_row = phase * TILE + threadIdx.y;
        int b_col = col;
        int b_index[3] = {batch, b_row, b_col};
        int b_pos = index_to_position(b_index, b_strides_all, num_dims);
      ...
      for (int ki = 0; ki < TILE; ki++) {
        out_value += a_shared[threadIdx.y][ki] * b_shared[ki][threadIdx.x];
      }
      __syncthreads();
    }
